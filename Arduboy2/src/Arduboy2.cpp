...
{
  bool ret = nextFrame();
//Throws error on Mega, commented it out
  //if (ret) {
...
bool Arduboy2Base::pressed(uint16_t buttons)
{
  return (buttonsState() & buttons) == buttons;
}

bool Arduboy2Base::notPressed(uint16_t buttons)
{
...
bool Arduboy2Base::justPressed(uint16_t button)
{
  return (!(previousButtonState & button) && (currentButtonState & button));
}

bool Arduboy2Base::justReleased(uint16_t button)
{
  return ((previousButtonState & button) && !(currentButtonState & button));
}
