...
{
#ifdef ARDUBOY_10
  //Modified to map to Mega 2560 pins and ports

  // Port A INPUT_PULLUP or HIGH
  PORTA |= _BV(LEFT_BUTTON_2_BIT) | _BV(RIGHT_BUTTON_2_BIT) |
         _BV(UP_BUTTON_2_BIT) | _BV(DOWN_BUTTON_2_BIT) |
         _BV(A_BUTTON_2_BIT) | _BV(B_BUTTON_2_BIT);
  //Port A inputs
  DDRA &= ~(_BV(LEFT_BUTTON_2_BIT) | _BV(RIGHT_BUTTON_2_BIT) |
          _BV(UP_BUTTON_2_BIT) | _BV(DOWN_BUTTON_2_BIT) |
          _BV(A_BUTTON_2_BIT) | _BV(B_BUTTON_2_BIT));

  // Port B INPUT_PULLUP or HIGH
  PORTB |= _BV(CS_BIT) | _BV(RED_LED_BIT) | _BV(GREEN_LED_BIT);
  // Port B INPUT or LOW (none)
  // Port B inputs
  DDRB &= ~(_BV(SPI_MISO_BIT));
  //Port B outputs
  DDRB |= _BV(RED_LED_BIT) | _BV(GREEN_LED_BIT) |  _BV(CS_BIT) | _BV(SPI_MOSI_BIT) |
       _BV(SPI_SCK_BIT) | _BV(SPI_SS_BIT);

  //Port E is speaker, handled by Audio class.
  ...
  // Port H INPUT_PULLUP or HIGH
  PORTH |= _BV(BLUE_LED_BIT) | _BV(A_BUTTON_BIT) | _BV(B_BUTTON_BIT);
  // Port H INPUT or LOW
  PORTH &= ~(_BV(RST_BIT));
  //Port H inputs
  DDRH &= ~(_BV(A_BUTTON_BIT) | _BV(B_BUTTON_BIT));
  //Port H outputs
  DDRH |= _BV(RST_BIT) | _BV(BLUE_LED_BIT);
...
//I am not using RGBs in my project.  I have commented out the code here.
//The code in these methods has NOT been updated for the Mega 2560.
...
uint16_t Arduboy2Core::buttonsState()
{
  uint16_t buttons;

  // Up, Right, Left, Down of Player 1
  buttons = ((~PINF) &
  ...
  if (bitRead(LEFT_BUTTON_2_PORTIN, LEFT_BUTTON_2_BIT) == 0) { buttons |= LEFT_BUTTON_2; }
  // Player 1 right button
  if (bitRead(RIGHT_BUTTON_2_PORTIN, RIGHT_BUTTON_2_BIT) == 0) { buttons |= RIGHT_BUTTON_2; }
  //Player 2 up button
  if (bitRead(UP_BUTTON_2_PORTIN, UP_BUTTON_2_BIT) == 0) { buttons |= UP_BUTTON_2; }
  // Player 2 down button
  if (bitRead(DOWN_BUTTON_2_PORTIN, DOWN_BUTTON_2_BIT) == 0) { buttons |= DOWN_BUTTON_2; }
  // Player 2 A button
  if (bitRead(A_BUTTON_2_PORTIN, A_BUTTON_2_BIT) == 0) { buttons |= A_BUTTON_2; }
  // Player 2 B button
  if (bitRead(B_BUTTON_2_PORTIN, B_BUTTON_2_BIT) == 0) { buttons |= B_BUTTON_2; }
...
{
  //Deleted code in here that caused Mega to crash
  init();
  setup();
